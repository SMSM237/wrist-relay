...
3. 새로 수집된 알림에서 채널·제목·본문 조건을 선택하고, 진동 프리셋의 테스트 알림을 워치에서 직접 확인합니다.
4. 규칙을 저장합니다. `규칙` 화면에서 나중에 조건·이름·프리셋을 다시 수정하거나 활성화·삭제할 수 있습니다.
5. 휴대폰만 조용히 하려면 Galaxy Wearable의 `휴대전화 알림 무음` 설정을 켭니다. 이 설정과 워치 진동은 앱이 대신 강제하거나 자동 검증할 수 없습니다.
